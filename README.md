# deceasedcraft-horde-pack

Дополнения для сервера DeceasedCraft (MC 1.20.1, Forge 47.4.0).

## Ресурспак: музыка ночи орды

`deceasedcraft-horde-pack.zip` — серверный ресурспак, клиент сам предлагает скачать его при входе.
Внутри два трека (namespace `deceasedhorde`):

| звук | файл |
| --- | --- |
| `deceasedhorde:horde.music.smaragdove_nebo` | `assets/deceasedhorde/sounds/horde/smaragdove_nebo.ogg` |
| `deceasedhorde:horde.music.plyashka_fragolino` | `assets/deceasedhorde/sounds/horde/plyashka_fragolino.ogg` |

Подключение (образ `itzg/minecraft-server`, env StatefulSet):

```yaml
- name: RESOURCE_PACK
  value: "https://raw.githubusercontent.com/o-e-lytvynskyi/deceasedcraft-horde-pack/main/deceasedcraft-horde-pack.zip"
- name: RESOURCE_PACK_SHA1
  value: "<sha1 zip>"
```

При обновлении пака пересобрать zip (`./build.sh`) и обновить `RESOURCE_PACK_SHA1`, иначе клиенты возьмут старую копию из кэша.

## KubeJS-скрипты (`kubejs/server_scripts/functions/`)

- `horde_siren.js` — за 200 тиков до орды тайтл «ОРДА», вой и музыка (категория звука «Враждебные существа», чтобы играла и с выключенной музыкой); треки чередуются, пока у игрока идёт орда,
  и выключаются, когда она кончается. Заодно синхронизирует орду между игроками: в день орды до окна старта ставит
  всем онлайн личный `nextDay` = сегодня, в остальные дни — общий `next_day` (The Hordes иначе сдвигает орду
  тому, кто зашёл в день орды). На каждую волну — одна из 15 угроз в общий чат, без повторов.
  Команды (op): `/hordesiren` — тест себе, `/hordesiren stop`, `/hordesiren threat` — следующая угроза себе,
  `/hordesiren status` — график и решения тика, только вызвавшему.
- `apoc_airdrop.js` — авиадропы Apocalypse Now: раз в час онлайна у случайного игрока и по команде
  `/apocairdrop common|military|medical`. Ящик падает в 30–60 блоках и дымит.

- `air_raid.js` — авианалёты: ковёр TNT 3 × 10 с шагом 5 блоков, за 30 с — предупреждение пилота в общий чат
  с координатами и курсом. Случайно (в среднем раз в 1,5 часа онлайна: рядом с игроком или по его координатам),
  рацией авиаудара (`apocalypsenow:military_radio` с NBT `airstrike:1b`, ПКМ в воздух — цель в 50 блоках по взгляду)
  и командами (op): `/airraid ahead|here|near`, `/airraid preview` (дым вместо TNT), `/airraid radio`.
  База (X 233..343, Z 3594..3672, +15 блоков) не бомбится никогда.

Копии лежат на сервере в `/data/kubejs/server_scripts/functions/`, применяются `/reload`.
Откат — удалить файл и `/reload`.
